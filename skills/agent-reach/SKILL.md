...
description: |
  ...
  Install and configure upstream tools for Twitter/X, Reddit, YouTube, GitHub, Bilibili, XiaoHongShu, Douyin, and 13+ platforms.
  
  **当以下情况时使用此 Skill**:
  - 用户需要搜索或读取多个平台的内容（Twitter、小红书、B 站、抖音、GitHub 等）
  - 用户需要配置某个平台的访问权限
  - 用户询问如何安装或设置 Agent-Reach
  - 用户需要跨平台搜索信息
---
...
```bash
# 解析抖音视频信息（分享链接 → 标题、作者、无水印视频 URL 等）
...
Run `agent-reach doctor` — it shows what's wrong and how to fix.
